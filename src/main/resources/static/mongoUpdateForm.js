fetch("http://localhost:8080/mongo/" + window.location.search.replace(/\?/g, '')).then(
	res => {
		res.json().then(
			data => {
				console.log(data);
                ...
                document.getElementById('isHighlight').value = data.isHighlight;
...
function update_object() {
	...
	if (objectName == '') {
		...
		var updateUrl = "http://localhost:8080/mongo/" + objectId;
		fetch(updateUrl, putMethod)
			...
			.then(res => {
				...
				if (res === "Success") {
					...
					window.location = "http://localhost:8080/mongoIndex.html";
				} else {
					alert(res);
				}
			})
