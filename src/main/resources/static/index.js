...
// Select the table (well, tbody)
  ...
  // get the cats
  let resp = await fetch('http://localhost:8080/museum');
...
function getObject() {
	fetch("http://localhost:8080/museum").then(
    	res => {
    		res.json().then(
    			data => {
    				console.log(data);
...
function addObject() {
	...
	var addUrl = "http://localhost:8080/museum/add/" + objectIdValue;
	fetch(addUrl)
		.then(res => res.text())
		.then(res => {
			console.log(res);
...
function updateObject() {
	...
	var updateUrl = "http://localhost:8080/museum/" + objectIdValue;
...
function deleteRow(r) {
	...
	var deleteUrl = "http://localhost:8080/museum/" + id;
...
function openUpdatePage(r) {
	var i = r.parentNode.parentNode.rowIndex;
	var id = document.getElementById("pager").rows[i].cells[0].innerHTML;
	window.location.href = "updateForm.html" + "?" + id;
}
